package com.sugar.library.ui.widget.countdown;
...
public class CountDownTextView extends androidx.appcompat.widget.AppCompatTextView {
    ...
    private static final String TIME_FORMAT_D_H_M_S = "%1$s天%2$02d:%3$02d:%4$02d";
    ...
    private String getFormatTime(long now) {
        ...
        switch (mTimeFlag) {
            case TIME_SHOW_D_H_M_S:
                if (day > 0) {
                    text = f.format(TIME_FORMAT_D_H_M_S, String.valueOf(day), hour, minute, seconds).toString();
                } else {
            ...
            case TIME_SHOW_S:
                ...
                break;
            default:
                text = f.format(TIME_FORMAT_H_M_S, 0, 0, seconds).toString();
                break;
        }
